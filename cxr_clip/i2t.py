...
def training(args, device, train_dataset, val_dataset):
    ...
    wandb.init(
        project="ITR-Finetuning-CXRCLIP",
    ...
    # define output dir
    weight_dir = Path("saved_models/cxrclip/i2t")
    weight_dir.mkdir(parents=True, exist_ok=True)
    weight_path_best = f"saved_models/cxrclip/i2t/{args.data_type}_{args.finetuning}_best.pth"
    weight_path_last = f"saved_models/cxrclip/i2t/{args.data_type}_{args.finetuning}_last.pth"
    ...
    model = CXRClip(model_config=ckpt_config["model"], all_loss_config=ckpt_config["loss"], tokenizer=tokenizer)
    ...
    if args.finetuning == "vision":
        ...
        lora_mode = "vision"
        model = LoRACXRClip(model_config=ckpt_config["model"], all_loss_config=ckpt_config["loss"], tokenizer=tokenizer, r=args.lora_rank, lora_mode=lora_mode)
        model.load_state_dict(ckpt["model"], strict=False)
        ...
        for name, param in model.named_parameters():
            ...
            if "text_encoder" in name:
                param.requires_grad = False
            if "text_projection.projection" in name:
                param.requires_grad = True
    ...
    if args.finetuning == "normal":
        if args.layer_wise:
            param_groups = [
                {"params": model.image_encoder.parameters(), "lr":args.lr_visual, "weight_decay":args.weight_decay_visual},
    ...
    max_length = ckpt_config["base"]["text_max_length"]
    ...
    with tqdm(desc=f"Epoch {1:4d}", total=args.epochs) as pbar:
        for epoch in range(1, args.epochs + 1):
            ...
            for case, sample in enumerate(train_dataloader):
                ...
                caption_token = tokenizer(captions[:max_length], padding="longest", truncation=True, return_tensors="pt", max_length=ckpt_config["base"]["text_max_length"]).to(device) 
                with torch.cuda.amp.autocast():
                    image_features = model.encode_image(image_input)   # [b, 512]
                    image_features = model.image_projection(image_features) if model.projection else image_features

                    logit_scale= model.logit_scale.exp()
                    text_features = model.encode_text(caption_token)   # [b, 512]
                    text_features = model.text_projection(text_features) if model.projection else text_features
                    

                image_features = image_features / image_features.norm(dim=-1, keepdim=True)
                text_features = text_features / text_features.norm(dim=-1, keepdim=True)

                logits_per_image = (logit_scale * (image_features @ text_features.T)).softmax(dim=-1)
                logits_per_text = (logit_scale * (text_features @ image_features.T)).softmax(dim=-1)
...
def prediction(args, device, test_dataset):
    wandb.login(key="c2f51df9b4bf2111388ccee68c2d41d301164d2c")
    wandb.init(
    project="ITR-Finetuning-CXRCLIP (Predictions)",
    name=args.wandbName,
    config={
    ...
    # load checkpoint
    if args.checkpoint_path is not None:
        loaded_checkpoint_path = f"saved_models/cxrclip/i2t/{args.checkpoint_path}"
        ...
        for case, sample in enumerate(test_dataloader):
            ...
            with torch.no_grad():
                with torch.cuda.amp.autocast():
                    ...
                    if case == 0:
                        text_inputs = tokenizer(texts_candidates[:max_length], padding="longest", truncation=True, return_tensors="pt", max_length=ckpt_config["base"]["text_max_length"]).to(device)
                        text_features = model.encode_text(text_inputs)
                        text_features = model.text_projection(text_features) if model.projection else text_features
                image_features /= image_features.norm(dim=-1, keepdim=True)
                text_features /= text_features.norm(dim=-1, keepdim=True)

                similarities = (model.logit_scale.exp() * (image_features @ text_features.T)).softmax(dim=-1)
    ...
    doc_dir = Path(f"saved_models/cxrclip/i2t/result_file")
    doc_dir.mkdir(parents=True, exist_ok=True)
    # save_root = os.path.join(doc_dir, f"results_{args.data_type}_{args.finetuning}.json")
    save_root = f"saved_models/cxrclip/i2t/result_file/results_{args.finetuning}_{args.data_type}.json"
