...
class VQADatasets(Dataset):
    def __init__(self, args, datasets, ckpt_config, dataset_name, mode="train"):
        ...
        if "SLAKE" in dataset_name:
            ...
            if args.RAD_split == "image_split":
                ...
                ans2label_path = "datasets/VQA/VQA-RAD/RAD_ans2label.pkl"
                label2ans_path = "datasets/VQA/VQA-RAD/RAD_label2ans.pkl"
        ...
        for entry in self.entries:
            tokens = self.tokenizer(entry["question"], padding="longest", truncation=True, return_tensors="pt", max_length=ckpt_config["base"]["text_max_length"])
...
class BilinearAttention(nn.Module):
    def __init__(self, feature_extractor, batch, gamma=4, num_ans_candidates=100, ans2label=None, ans2vector=None):
        ...
        self.v_dim = self.feature_extractor.image_encoder.out_dim
    ...
    def forward(self, image, question_token):
        # image feature input
        image_features = self.feature_extractor.encode_image(image).unsqueeze(1)   # [batch: 3, 1, output_dim/emb_dim: 2048]
        # textual information
        text_features = self.feature_extractor.text_encoder(question_token["input_ids"])   # [3, content_length: 18, embedding: 768]
...
def training(args, device, train_dataset, val_dataset):
    ...
    wandb.init(
        project="VQA-Finetuning-CXRCLIP",
    ...
    # define output dir
    weight_dir = Path("saved_models/cxrclip/vqa")
    weight_dir.mkdir(parents=True, exist_ok=True)
    weight_path_best = f"saved_models/cxrclip/vqa/{args.dataset}_{args.finetuning}_best.pth"
    weight_path_last = f"saved_models/cxrclip/vqa/{args.dataset}_{args.finetuning}_last.pth"
    ...
    clip_model = CXRClip(model_config=ckpt_config["model"], all_loss_config=ckpt_config["loss"], tokenizer=tokenizer, classifier=None)
    ...
    # prepare DataLoader
    train_data_prepared = VQADatasets(args, train_dataset, ckpt_config, dataset_name=args.dataset, mode="train")
    val_data_prepared = VQADatasets(args, val_dataset, ckpt_config, dataset_name=args.dataset, mode="train")
    ...
    if args.finetuning == "BAN":
        ...
        clip_model = LoRACXRClip(model_config=ckpt_config["model"], all_loss_config=ckpt_config["loss"], tokenizer=tokenizer, r=args.lora_rank, lora_mode="vision_projection", classifier=None)
    ...
    with tqdm(desc=f"Epoch {1:4d}", total=args.epochs) as pbar:
        for epoch in range(1, args.epochs + 1):
            ...
            for case, batch in enumerate(train_dataloader):
                image = batch.image.to(device)   # [3, 3, 224, 224]
                question_tokens = tokenizer(batch.question, padding="longest", truncation=True, return_tensors="pt", max_length=ckpt_config["base"]["text_max_length"]).to(device)   # [3, 18]
                target = batch.target.to(device)   # [3. 459]
...
def evaluation(args, device, test_dataset, checkpoint_path=None):
    wandb.login(key="c2f51df9b4bf2111388ccee68c2d41d301164d2c")
    wandb.init(
    project="VQA-Finetuning-CXRCLIP (Predictions)",
    name=args.wandbName,
    config={
        "task": "VQA-prediction",
        "architecture": "CXRCLIP(ResNet50 & Bert)",
    ...
    test_data_prepared = VQADatasets(args, test_dataset, ckpt_config, dataset_name=args.dataset, mode="test")
    ...
    if checkpoint_path is not None:
        ...
        checkpoint_path = f"saved_models/cxrclip/vqa/{checkpoint_path}"
    ...
    doc_dir = Path("saved_models/cxrclip/vqa/result_files")
    doc_dir.mkdir(parents=True, exist_ok=True)
    outfile = f"saved_models/cxrclip/vqa/result_files/results_{args.dataset}_{finetuning}.json"
    if not os.path.exists(os.path.dirname(outfile)):
        os.makedirs(os.path.dirname(outfile))
    json.dump(result, open(outfile, "w"))

    for i in question_types_result:
        pd.DataFrame(question_types_result[i]).transpose().to_csv(f"saved_models/cxrclip/vqa/result_files/{args.dataset}_{finetuning}_question_type_{i}.csv")

    df = pd.DataFrame(target_results)
    df.to_csv(f"saved_models/cxrclip/vqa/result_files/{args.dataset}_{finetuning}_targeted_predictions.csv", index=False)
    wandb.finish()
