...
from .modules import load_image_encoder, load_lora_image_encoder, load_projection_head, load_lora_projection_head, load_text_encoder, load_image_classifier
...
class CXRClip(nn.Module):
    def __init__(self, model_config: Dict, all_loss_config: Dict, tokenizer: PreTrainedTokenizer = None, classifier=None):
        super().__init__()
        ...
        if classifier is not None:
            self.classifier = LinearClassifier(self.image_encoder.out_dim, classifier)
    ...
    def encode_text(self, text_tokens):
        text_tokens_f = text_tokens["input_ids"]
        text_features = self.text_encoder(text_tokens_f)
...
class LoRACXRClip(nn.Module):
    def __init__(self, model_config: Dict, all_loss_config: Dict, tokenizer: PreTrainedTokenizer = None, r = 4, lora_mode = None, classifier=None):
        super().__init__()
        self.tokenizer = tokenizer
        if lora_mode is not None and "vision" in lora_mode:
            self.image_encoder = load_lora_image_encoder(model_config["image_encoder"], r=r)
        else:
            self.image_encoder = load_image_encoder(model_config["image_encoder"])
        self.text_encoder = load_text_encoder(model_config["text_encoder"], vocab_size=tokenizer.vocab_size)
        self.text_pooling = model_config["text_encoder"]["pooling"]
        ...
        if self.projection:
            if "projection" in lora_mode:
                self.image_projection = load_lora_projection_head(
                    embedding_dim=self.image_encoder.out_dim, config_projection_head=model_config["projection_head"], r=r
                )
                self.text_projection = load_lora_projection_head(
                    embedding_dim=self.text_encoder.out_dim, config_projection_head=model_config["projection_head"], r=r
                )
            else:
                self.image_projection = load_projection_head(
                    embedding_dim=self.image_encoder.out_dim, config_projection_head=model_config["projection_head"]
