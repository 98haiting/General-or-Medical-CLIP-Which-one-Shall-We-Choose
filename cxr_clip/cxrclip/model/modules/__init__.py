...
from .image_encoder import HuggingfaceImageEncoder, ResNet50, LoRAResNet50
from .projection import LinearProjectionHead, MLPProjectionHead, LoRAMLPProjectionHead, LoRALinearProjectionHead
...
def load_lora_image_encoder(config_image_encoder: Dict, r=4):
    if config_image_encoder["source"].lower() == "huggingface":
        cache_dir = config_image_encoder["cache_dir"] if "cache_dir" in config_image_encoder else "~/.cache/huggingface/hub"
        gradient_checkpointing = (
            config_image_encoder["gradient_checkpointing"] if "gradient_checkpointing" in config_image_encoder else False
        ...
        _image_encoder = LoRAResNet50(r=r)
...
def load_lora_projection_head(embedding_dim: int, config_projection_head: Dict, r=4):
    if config_projection_head["name"].lower() == "mlp":
        projection_head = LoRAMLPProjectionHead(
            embedding_dim=embedding_dim, projection_dim=config_projection_head["proj_dim"], dropout=config_projection_head["dropout"], r=r
        )
    elif config_projection_head["name"].lower() == "linear":
        projection_head = LoRALinearProjectionHead(embedding_dim=embedding_dim, projection_dim=config_projection_head["proj_dim"], r=r)
    else:
        raise KeyError(f"Not supported text encoder: {config_projection_head}")
    return projection_head


def load_image_classifier(config_image_classifier: Dict, feature_dim: int, n_class: int):
    if config_image_classifier["name"].lower() == "linear":
        _image_classifier = LinearClassifier(feature_dim=feature_dim, num_class=n_class)
    else:
