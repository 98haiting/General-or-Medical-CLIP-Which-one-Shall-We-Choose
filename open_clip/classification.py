...
class OpenCLIP_Classifier(torch.nn.Module):
...
def load_model(device, model_name, checkpoint=None):
    ...
    if checkpoint is not None:
        checkpoint_path = f"saved_models/open_clip/classification/{checkpoint}"
        ...
        if "clf" in checkpoint:
            ...
            if "breastmnist" in checkpoint or "pneumoniamnist" in checkpoint:
                ...
                nClasses = 11
            model = OpenCLIP_Classifier(model, num_classes=nClasses, task="binary-class")
...
def predictor(datasets_test, model, tokenizer, preprocess, device):
            ...
            for case, (image, class_id) in enumerate(dataset_test):
                ...
                if args.checkpoint is not None:
                    if "clf" in args.checkpoint:
                        ...
                        text_inputs = tokenizer(list(dataset_test.info['label'].values())).to(device)
                        with torch.no_grad():
    ...
    save_doc = Path("saved_models/open_clip/classification")
    save_doc.mkdir(parents=True, exist_ok=True)
    if args.checkpoint_path:
        outfile = f"saved_models/open_clip/classification/prediction_{args.dataset}.csv"
    else:
        outfile = f"saved_models/open_clip/classification/prediction_wo_{args.dataset}.csv"
...
if __name__ == "__main__":
    ...
    wandb.init(
    project="Classification-Finetuning-OpenCLIP (Predictions)",
    name=args.wandbName,
    config={
    ...
    base_path = "saved_models/open_clip/classification/*"
    if args.checkpoint_path:
        for file in glob.glob(base_path):
            ...
            model, preprocess, tokenizer = load_model(device, "open_clip", checkpoint=args.checkpoint)
            predictor(datasets_test, model, tokenizer, preprocess, device)
    else:
        datasets_test = download_datasets(args)   
        model, preprocess, tokenizer = load_model(device, "open_clip")
        # prediction
