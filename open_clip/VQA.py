...
from vqa.utils import trim_collate
from typing import NamedTuple
# from monai.transforms import LoadImage, Resize, CenterSpatialCrop, ToTensor, NormalizeIntensity, Compose, RandFlip, RandRotate, RandZoom, ScaleIntensity
...
def generate_ans_label(input_path, output_path):
    ...
    pickle.dump(ans2label, open(output_path+"RAD_ans2label.pkl", "wb"))
    pickle.dump(label2ans, open(output_path+"RAD_label2ans.pkl", "wb"))
...
class VQADatasets(Dataset):
    def __init__(self, args, datasets, preprocess, dataset_name, mode="train", tokenizer=None):
        ...
        #     ToTensor(),
        #     Resize(spatial_size=224, mode="bicubic"),
        #     CenterSpatialCrop(roi_size=224),
        #     ScaleIntensity(),
        ...
        for entry in self.entries:
            tokens = tokenizer(entry["question"])   # shape of token: [1, 77]
...
class BilinearAttention(nn.Module):
    def __init__(self, feature_extractor, batch, gamma=4, num_ans_candidates=100, ans2label=None, ans2vector=None):
        ...
        # self.embedding_space = torch.zeros((num_ans_candidates, self.v_dim))
        # self.answer = []
        # for answer, embedding in ans2vector.items():
        #     idx = ans2label[answer]
        #     self.embedding_space[idx] = normalize(torch.from_numpy(np.array(embedding).squeeze()), dim=-1)


    def forward(self, image, question_token):
        # image feature input
        image_features = self.feature_extractor.encode_image(image).unsqueeze(1)   # [batch: 3, output_dim/emb_dim: 512]
        
        # textual information
        output = self.feature_extractor.token_embedding(question_token)   # [3, q_len: 77, emd_dim: 512]
        output = output + self.feature_extractor.positional_embedding   # [3, q_len: 77, emd_dim: 512]
        output = output.permute(1, 0, 2)   # [77, 3, 512]
        output = self.feature_extractor.transformer(output)   # [77, 3, 512]
        text_features = output.permute(1, 0, 2)   # [3, 77, 512]
...
def training(args, device, train_dataset, val_dataset):
    ...
    wandb.init(
        project="VQA-Finetuning-OpenCLIP",
    ...
    # define output dir
    weight_dir = Path("saved_models/open_clip/vqa")
    weight_dir.mkdir(parents=True, exist_ok=True)
    weight_path_best = f"saved_models/open_clip/vqa/{args.dataset}_{args.finetuning}_best.pth"
    weight_path_last = f"saved_models/open_clip/vqa/{args.dataset}_{args.finetuning}_last.pth"
    ...
    # prepare DataLoader
    train_data_prepared = VQADatasets(args, train_dataset, preprocess, dataset_name=args.dataset, mode="train", tokenizer=tokenizer)
    val_data_prepared = VQADatasets(args, val_dataset, preprocess, dataset_name=args.dataset, mode="train", tokenizer=tokenizer)
    ...
    # ans2vector = generate_ans_vector(clip_model, train_data_prepared.ans2label, output_path=f"datasets/VQA/{args.dataset}/ans2vector.pkl")
    ...
    if args.finetuning == "BAN":
        ...
        clip_model = build_LoRA_model(clip_model.state_dict(), r=args.lora_rank, lora_mode="vision_projection")
        model = BilinearAttention(feature_extractor=clip_model, batch=args.batch_size, gamma=args.gamma, num_ans_candidates=train_data_prepared.num_ans_candidates, ans2label=train_data_prepared.ans2label, ans2vector=ans2vector)
        sum_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print(f"after finetuning: {sum_params}")
    ...
    with tqdm(desc=f"Epoch {1:4d}", total=args.epochs) as pbar:
        for epoch in range(1, args.epochs + 1):
            ...
            for case, batch in enumerate(train_dataloader):
                ...
                with torch.cuda.amp.autocast():
                    ...
                    # preds_embeddings = model.classifier(features)   # [3, 512]
                    preds = model.classifier(features)   # [Batch, 459]

                # calculate cosine similarity between predicted embeddings and all answer embeddings
                # preds_embeddings = normalize(preds_embeddings, dim=-1)   # [batch, 512]
                # similarities = (preds_embeddings @ model.embedding_space.to(device).to(preds_embeddings.dtype).T)   # [batch, 512] @ [num_classes, 512] -> [batch, num_classes]

                # loss = criterion(similarities, target)
            ...
            for case, batch in enumerate(val_dataloader):
                ...
                with torch.no_grad():
                    with torch.cuda.amp.autocast():
                        ...
                        # preds_embeddings = model.classifier(features)
                        preds = model.classifier(features)
                    
                    # calculate cosine similarity between predicted embeddings and all answer embeddings
                    # preds_embeddings = normalize(preds_embeddings, dim=-1)   # [batch, 512]
                    # similarities = (preds_embeddings @ model.embedding_space.T)   # [batch, 512] @ [num_classes, 512] -> [batch, num_classes]
        ...
        # ans2vector = generate_ans_vector(model.feature_extractor, train_data_prepared.ans2label, output_path=f"datasets/VQA/VQA-RAD/ans2vector_{args.dataset}_{args.finetuning}_last.pkl")
...
def evaluation(args, device, test_dataset, checkpoint_path=None):
    wandb.login(key="c2f51df9b4bf2111388ccee68c2d41d301164d2c")
    wandb.init(
    project="VQA-Finetuning-OpenCLIP (Predictions)",
    name=args.wandbName,
    config={
        "task": "VQA-prediction",
    ...
    test_data_prepared = VQADatasets(args, test_dataset, preprocess, dataset_name=args.dataset, mode="test", tokenizer=tokenizer)
    ...
    if checkpoint_path is not None:
        ...
        checkpoint_path = f"saved_models/open_clip/vqa/{checkpoint_path}"
    ...
    with tqdm(desc=f"case {0:5d}", total=len(test_dataloader), unit="case") as pbar:
        for case, batch in enumerate(test_dataloader):
            ...
            # question_tokens = batch["q_token"].squeeze(1).to(device)
            # target = batch["target"].to(device)
            # answers = batch["answer"]["answer_text"]
            image = batch.image.to(device)
            question_tokens = batch.q_token.squeeze(1).to(device)
            target = batch.target.to(device)
    ...
    doc_dir = Path("saved_models/open_clip/vqa/result_files")
    doc_dir.mkdir(parents=True, exist_ok=True)
    outfile = f"saved_models/open_clip/vqa/result_files/results_{args.dataset}_{finetuning}.json"
    if not os.path.exists(os.path.dirname(outfile)):
        os.makedirs(os.path.dirname(outfile))
    json.dump(result, open(outfile, "w"))

    for i in question_types_result:
        pd.DataFrame(question_types_result[i]).transpose().to_csv(f"saved_models/open_clip/vqa/result_files/{args.dataset}_{finetuning}_question_type_{i}.csv")

    df = pd.DataFrame(target_results)
    df.to_csv(f"saved_models/open_clip/vqa/result_files/{args.dataset}_{finetuning}_targeted_predictions.csv", index=False)
    wandb.finish()
...
if __name__ == '__main__':
    ...
    # input_path = "datasets/VQA/VQA-RAD/image_anstype_split/"
    # output_path = "datasets/VQA/VQA-RAD/image_anstype_split/"
    # generate_ans_label(input_path, output_path)

    # some of the labels in test_target are none
    # generate dictionary
    # output_path = "datasets/VQA/SLAKE/SLAKE_Dictionary.pkl"
    # input_path = "datasets/VQA/SLAKE/"
    # Dictionary = Dictionary()
    # generate_dic(Dictionary, input_path, output_path)

    # generate ans2label and label2ans
    # input_path = "datasets/VQA/SLAKE/"
    # output_path = "datasets/VQA/SLAKE/"
    # generate_ans_label(input_path=input_path, output_path=output_path)

    # generate image name_to_idx
    # input_path = "datasets/VQA/SLAKE/"
    # output_path = "datasets/VQA/SLAKE/SLAKE_img2idx.pkl"
    # generate_imgid_idx(input_path, output_path)

    # args.dataset = "VQA-RAD"
    # args.dataset_path = "datasets/VQA/VQA-RAD"
    # args.batch_size = 3
    # args.epochs = 1
    # args.mode = "train"
    # args.finetuning = "normal-clip"
    # args.data_augmentation = True
    # args.layer_wise = True
    # args.checkpoint_path = "SLAKE_BAN_best.pth"
