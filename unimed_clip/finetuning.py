...
import argparse
from _open_clip import create_model_and_transforms, HFTokenizer
import torch 
...
class MedMNISTDataset(Dataset):
    def __init__(self, datasets, preprocess, percentage, image_size: int, transform=False):
...
class UniMed_CLIP_Classifier(torch.nn.Module):
...
def main_worker(datasets_train, datasets_val, device, args):
    ...
    wandb.init(
        project="Classification-Finetuning-UniMedCLIP",
    ...
    weight_dir = Path("saved_models/unimed_clip/classification")
    weight_dir.mkdir(parents=True, exist_ok=True)
    if args.combination:
        if args.classifier:
            weight_path_best = f"saved_models/unimed_clip/classification/{args.datasets_list}_{args.finetuning}_clf_best.pth"
            weight_path_last = f"saved_models/unimed_clip/classification/{args.datasets_list}_{args.finetuning}_clf_last.pth"
        else:
            weight_path_best = f"saved_models/unimed_clip/classification/{args.datasets_list}_{args.finetuning}_best.pth"
            weight_path_last = f"saved_models/unimed_clip/classification/{args.datasets_list}_{args.finetuning}_last.pth"
    else:
        if args.classifier:
            weight_path_best = f"saved_models/unimed_clip/classification/{args.dataset}_{args.finetuning}_clf_best.pth"
            weight_path_last = f"saved_models/unimed_clip/classification/{args.dataset}_{args.finetuning}_clf_last.pth"
        else:
            weight_path_best = f"saved_models/unimed_clip/classification/{args.dataset}_{args.finetuning}_best.pth"
            weight_path_last = f"saved_models/unimed_clip/classification/{args.dataset}_{args.finetuning}_last.pth"
    ...
    if args.classifier:
        model = UniMed_CLIP_Classifier(model, num_classes=len(datasets_train[0].info['label']), task=task)
    ...
    if args.distributed:
        train_sampler = torch.utils.data.distributed.DistributedSampler(MedMNISTDataset(datasets_train, preprocess=preprocess, image_size=224))
        val_sampler = torch.utils.data.distributed.DistributedSampler(MedMNISTDataset(datasets_val, preprocess=preprocess, image_size=224))
        ...
        train_dataloader = DataLoader(MedMNISTDataset(datasets_train, percentage=args.percentage, preprocess=preprocess, image_size=224), batch_size=args.batch_size, shuffle=True)
        val_dataloader = DataLoader(MedMNISTDataset(datasets_val, percentage=args.percentage, preprocess=preprocess, image_size=224), batch_size=args.batch_size, shuffle=False)
        ...
        for epoch in range(1, args.epochs + 1): 
            ...
            for case, (image, label, text) in enumerate(train_dataloader):
                ...
                if args.classifier:
                    ...
                    target_tokens = tokenizer(text).to(device)
                    
                    with torch.cuda.amp.autocast(): 
                        image_features = model.encode_image(image)   # (b, 512)
                        text_features = model.encode_text(target_tokens)   # (b, 512)
