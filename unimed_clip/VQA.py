...
class VQADatasets(Dataset):
    def __init__(self, args, datasets, preprocess, model, dataset_name, mode="train", tokenizer=None):
...
class BilinearAttention(nn.Module):
    ...
    def forward(self, image, question_token):
        # image feature input
        image_features = self.feature_extractor.encode_image(image).unsqueeze(1)   # [batch: 3, output_dim/emb_dim: 768]
        if isinstance(image_features, dict):
            image_features = image_features["image_features"].unsqueeze(1)   # [batch: 3, 1, output_dim/emb_dim: 768]
        
        # textual information
        text_features = self.feature_extractor.encode_text(question_token).unsqueeze(1)
        # attention mechanism
...
def training(args, device, train_dataset, val_dataset):
    ...
    wandb.init(
        project="VQA-Finetuning-UnimedCLIP",
    ...
    # define output dir
    weight_dir = Path("saved_models/unimed_clip/vqa")
    weight_dir.mkdir(parents=True, exist_ok=True)
    weight_path_best = f"saved_models/unimed_clip/vqa/{args.dataset}_{args.finetuning}_best.pth"
    weight_path_last = f"saved_models/unimed_clip/vqa/{args.dataset}_{args.finetuning}_last.pth"
    ...
    # prepare DataLoader
    train_data_prepared = VQADatasets(args, train_dataset, preprocess, model=clip_model, dataset_name=args.dataset, mode="train", tokenizer=tokenizer)
    val_data_prepared = VQADatasets(args, val_dataset, preprocess, model=clip_model, dataset_name=args.dataset, mode="train", tokenizer=tokenizer)
    ...
    if args.finetuning == "BAN":
        ...
        for name, param in model.named_parameters():
            if "feature_extractor.text_encoder.proj" in name:
                param.requires_grad = True
            if "feature_extractor.logit_scale" in name:
                param.requires_grad = True 
            if "feature_extractor.visual.ln_post" in name:
                param.requires_grad = True
            if "feature_extractor.visual.proj" in name:
                param.requires_grad = True
        sum_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print(f"after finetuning: {sum_params}")
...
def evaluation(args, device, test_dataset, checkpoint_path=None):
    wandb.login(key="c2f51df9b4bf2111388ccee68c2d41d301164d2c")
    wandb.init(
    project="VQA-Finetuning-UnimedCLIP (Predictions)",
    name=args.wandbName,
    config={
        "task": "VQA-prediction",
        "architecture": "UnimedCLIP(ResNet50 & Bert)",
    ...
    test_data_prepared = VQADatasets(args, test_dataset, preprocess=preprocess, model=model, dataset_name=args.dataset, mode="test", tokenizer=tokenizer)
    ...
    if checkpoint_path is not None:
        ...
        checkpoint_path = f"saved_models/unimed_clip/vqa/{checkpoint_path}"
        ...
        if "lora" in finetuning:
            ...
            model = BilinearAttention(feature_extractor=model, batch=args.batch_size, gamma=args.gamma, num_ans_candidates=test_data_prepared.num_ans_candidates, ans2label=test_data_prepared.ans2label, ans2vector=ans2vector)
        else:
            model = BilinearAttention(feature_extractor=model, batch=1, gamma=args.gamma, num_ans_candidates=test_data_prepared.num_ans_candidates, ans2label=test_data_prepared.ans2label, ans2vector=ans2vector)
    ...
    doc_dir = Path("saved_models/unimed_clip/vqa/result_files")
    doc_dir.mkdir(parents=True, exist_ok=True)
    outfile = f"saved_models/unimed_clip/vqa/result_files/results_{args.dataset}_{finetuning}.json"
    if not os.path.exists(os.path.dirname(outfile)):
        os.makedirs(os.path.dirname(outfile))
    json.dump(result, open(outfile, "w"))

    for i in question_types_result:
        pd.DataFrame(question_types_result[i]).transpose().to_csv(f"saved_models/unimed_clip/vqa/result_files/{args.dataset}_{finetuning}_question_type_{i}.csv")

    df = pd.DataFrame(target_results)
    df.to_csv(f"saved_models/unimed_clip/vqa/result_files/{args.dataset}_{finetuning}_targeted_predictions.csv", index=False)
...
if __name__ == '__main__':
    ...
    # args.dataset = "SLAKE"
    # args.dataset_path = "datasets/VQA/SLAKE"
    # args.batch_size = 3
    # args.epochs = 1
    # args.mode = "train"
