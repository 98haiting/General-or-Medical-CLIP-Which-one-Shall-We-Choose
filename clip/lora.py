...
class LoRAResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, r=4):
        ...
        self.attn = lora_ft.MultiheadAttention(d_model, n_head, r=r) # LoRA rank set as 4
        self.ln_1 = LayerNorm(d_model)
        self.mlp = nn.Sequential(OrderedDict([
            ("c_fc", lora_ft.Linear(d_model, d_model * 4, r=r)),
            ("gelu", QuickGELU()),
            ("c_proj", lora_ft.Linear(d_model * 4, d_model, r=r))
        ]))
        self.ln_2 = LayerNorm(d_model)
        self.attn_mask = attn_mask
...
class LoRAVisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, r: int):
        ...
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        self.conv1 = lora_ft.Conv2d(in_channels=3, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False)
...
class LoRACLIP(nn.Module): 
    def __init__(self,
        ...
        if "text" in lora_mode or "projection":
            self.lora_text_projection = lora_ft.Linear(transformer_width, embed_dim, r=r, bias=False)
            self.token_embedding = lora_ft.Embedding(vocab_size, transformer_width, r=r)
