...
class MedMNISTDataset(Dataset):
    def __init__(self, datasets, percentage, preprocess, transform=False):
    ...
    def __getitem__(self, idx):
        ...
        text_token = clip.tokenize(f"this is a photo of {text}")
...
def main_worker(datasets_train, datasets_val, device, args):
    ...
    wandb.init(
        project="Classification-Finetuning-CLIP (TEST FOR BETTER)",
    ...
    weight_dir = Path("saved_models/clip/classification")
    weight_dir.mkdir(parents=True, exist_ok=True)
    if args.combination:
        if args.classifier:
            weight_path_best = f"saved_models/clip/classification/{args.datasets_list}_{args.finetuning}_clf_best.pth"
            weight_path_last = f"saved_models/clip/classification/{args.datasets_list}_{args.finetuning}_clf_last.pth"
        else:
            weight_path_best = f"saved_models/clip/classification/{args.datasets_list}_{args.finetuning}_best.pth"
            weight_path_last = f"saved_models/clip/classification/{args.datasets_list}_{args.finetuning}_last.pth"
    else:
        if args.classifier:
            weight_path_best = f"saved_models/clip/classification/{args.dataset}_{args.finetuning}_clf_best.pth"
            weight_path_last = f"saved_models/clip/classification/{args.dataset}_{args.finetuning}_clf_last.pth"
        else:
            weight_path_best = f"saved_models/clip/classification/{args.dataset}_{args.finetuning}_best.pth"
            weight_path_last = f"saved_models/clip/classification/{args.dataset}_{args.finetuning}_last.pth"
    ...
    if args.classifier:
        model = CLIP_Classifier(model, num_classes=len(datasets_train[0].info['label']), task=task)
    ...
    if args.distributed:
        train_sampler = torch.utils.data.distributed.DistributedSampler(MedMNISTDataset(datasets_train, preprocess, transform=args.augmentation))
        val_sampler = torch.utils.data.distributed.DistributedSampler(MedMNISTDataset(datasets_val, preprocess, transform=args.augmentation))
        ...
        train_dataloader = DataLoader(MedMNISTDataset(datasets_train, args.percentage, preprocess, transform=args.augmentation), batch_size=args.batch_size, shuffle=True)
        val_dataloader = DataLoader(MedMNISTDataset(datasets_val, args.percentage, preprocess, transform=args.augmentation), batch_size=args.batch_size, shuffle=False)
