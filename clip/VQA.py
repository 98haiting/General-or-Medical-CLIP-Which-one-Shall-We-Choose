...
class VQADatasets(Dataset):
    def __init__(self, args, datasets, preprocess, dataset_name, mode="train"):
        ...
        for entry in self.entries:
            tokens = clip.tokenize(entry["question"])   # shape of token: [1, 77]
...
def training(args, device, train_dataset, val_dataset):
    ...
    wandb.init(
        project="VQA-Finetuning-CLIP",
        name=f"{args.dataset}-{args.finetuning}",
        config={
            "task": "classification-finetuning",
            "architecture": "CLIP(ViT-B/32 & Transformer)",
    ...
    # define output dir
    weight_dir = Path("saved_models/clip/vqa")
    weight_dir.mkdir(parents=True, exist_ok=True)
    weight_path_best = f"saved_models/clip/vqa/{args.dataset}_{args.finetuning}_best.pth"
    weight_path_last = f"saved_models/clip/vqa/{args.dataset}_{args.finetuning}_last.pth"
    ...
    # prepare DataLoader
    train_data_prepared = VQADatasets(args, train_dataset, preprocess, dataset_name=args.dataset, mode="train")
    val_data_prepared = VQADatasets(args, val_dataset, preprocess, dataset_name=args.dataset, mode="train")
...
def evaluation(args, device, test_dataset, checkpoint_path=None):
    wandb.login(key="c2f51df9b4bf2111388ccee68c2d41d301164d2c")
    wandb.init(
    project="VQA-Finetuning-CLIP (Predictions)",
    name=args.wandbName,
    config={
        "task": "VQA-prediction",
    ...
    test_data_prepared = VQADatasets(args, test_dataset, preprocess, dataset_name=args.dataset, mode="test")
    ...
    if checkpoint_path is not None:
        ...
        checkpoint_path = f"saved_models/clip/vqa/{checkpoint_path}"
    ...
    doc_dir = Path("saved_models/clip/vqa/result_files")
    doc_dir.mkdir(parents=True, exist_ok=True)
    outfile = f"saved_models/clip/vqa/result_files/results_{args.dataset}_{finetuning}.json"
    if not os.path.exists(os.path.dirname(outfile)):
        os.makedirs(os.path.dirname(outfile))
    json.dump(result, open(outfile, "w"))

    for i in question_types_result:
        pd.DataFrame(question_types_result[i]).transpose().to_csv(f"saved_models/clip/vqa/result_files/{args.dataset}_{finetuning}_question_type_{i}.csv")

    df = pd.DataFrame(target_results)
    df.to_csv(f"saved_models/clip/vqa/result_files/{args.dataset}_{finetuning}_targeted_predictions.csv", index=False)
    wandb.finish()
