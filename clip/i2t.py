...
def training(args, device, train_dataset, val_dataset):
    ...
    # define output dir
    weight_dir = Path("saved_models/clip/i2t")
    weight_dir.mkdir(parents=True, exist_ok=True)
    weight_path_best = f"saved_models/clip/i2t/{args.data_type}_{args.finetuning}_best.pth"
    weight_path_last = f"saved_models/clip/i2t/{args.data_type}_{args.finetuning}_last.pth"
    ...
    with tqdm(desc=f"Epoch {1:4d}", total=args.epochs) as pbar:
        for epoch in range(1, args.epochs + 1):
            ...
            for case, sample in enumerate(train_dataloader):
                ...
                caption_token = torch.cat([clip.tokenize(caption[:max_length]) for caption in captions]).to(device)   # [b, 77]
            ...
            for case, sample in enumerate(val_dataloader):
                ...
                caption_token = torch.cat([clip.tokenize(caption[:max_length]) for caption in captions]).to(device)
...
def prediction(args, device, test_dataset):
    wandb.login(key="c2f51df9b4bf2111388ccee68c2d41d301164d2c")
    wandb.init(
    project="ITR-Finetuning-CLIP (Predictions)",
    name=args.wandbName,
    config={
    ...
    # load checkpoint
    if args.checkpoint_path is not None:
        checkpoint_path = f"saved_models/clip/i2t/{args.checkpoint_path}"
        ...
        for case, sample in enumerate(test_dataloader):
            ...
            with torch.no_grad():
                with torch.cuda.amp.autocast():
                    ...
                    if case == 0:
                        text_inputs = torch.cat([clip.tokenize(caption[:max_length]) for caption in texts_candidates]).to(device)
    ...
    doc_dir = Path(f"saved_models/clip/i2t/result_file")
    doc_dir.mkdir(parents=True, exist_ok=True)
    save_root = f"saved_models/clip/i2t/result_file/results_{args.finetuning}_{args.data_type}.json"
