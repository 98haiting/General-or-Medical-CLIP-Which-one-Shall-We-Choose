...
def load_model(device, model_name, checkpoint=None):
    ...
    if checkpoint is not None:
        checkpoint_path = f"saved_models/clip/classification/{checkpoint}"
        ...
        if "clf" in checkpoint:
            ...
            if "breastmnist" in checkpoint or "pneumoniamnist" in checkpoint:
                ...
                nClasses = 11
            model = CLIP_Classifier(model, num_classes=nClasses, task="binary-class")
...
# predictions
def predictor(datasets_test, model, preprocess, device):
    model.eval()
            ...
            for case, (image, class_id) in enumerate(dataset_test):
                ...
                if args.checkpoint is not None:
                    if "clf" in args.checkpoint:
                        ...
                        text_inputs = torch.cat([clip.tokenize(f"this is a photo of {c}") for c in dataset_test.info['label'].values()]).to(device)
                        with torch.no_grad():
    ...
    save_doc = Path("saved_models/clip/classification")
    save_doc.mkdir(parents=True, exist_ok=True)
    outfile = f"saved_models/clip/classification/prediction_{args.dataset}.csv"
...
if __name__ == "__main__":
    ...
    wandb.init(
    project="Classification-Finetuning-CLIP (Predictions)",
    name=args.wandbName,
    config={
    ...
    base_path = "saved_models/clip/classification/*"
    if args.checkpoint_path:
        for file in glob.glob(base_path):
            ...
            model, preprocess = load_model(device, "clip", checkpoint=args.checkpoint)
            predictor(datasets_test, model, preprocess, device)
    else:
        datasets_test = download_datasets(args)   
        model, preprocess = load_model(device, "clip")
        # prediction
