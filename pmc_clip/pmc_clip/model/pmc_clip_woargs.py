...
import loralib.layers as lora_ft
...
from .blocks import Bottleneck, AttentionPool2d, ResNet, ModifiedResNet, LayerNorm, QuickGELU, ResidualAttentionBlock,\
                    Transformer, LoRAResNet, LoRAModifiedResNet
    ...
    def __init__(
        ...
        if vision_cfg.timm_model_name:
            ...
            if "vision" in lora_mode:
                VisualBackbone = {
                    "RN50": LoRAResNet,
                    "ModifiedRN50": LoRAModifiedResNet,
                }[vision_cfg.backbone]
                vision_heads = vision_cfg.width * 32 // vision_cfg.head_width
        ...
        if "projection" in lora_mode:
            self.lora_text_projection = lora_ft.Linear(text_cfg.width, embed_dim, r=lora_rank)
        else:
            self.lora_text_projection = nn.Linear(text_cfg.width, embed_dim)
